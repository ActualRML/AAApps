...
contract IntegratedFuzz is Test {
    ...
    function setUp() public {
        ...
        vm.mockCall(ep, abi.encodeWithSignature("depositTo(address)"), abi.encode());
        ...
        vm.mockCall(oracle, abi.encodeWithSelector(IAggregatorV3Local.decimals.selector), abi.encode(uint8(8)));
        ...
        bytes memory aInitData =
            abi.encodeWithSelector(SmartAccount.initialize.selector, user, guardians, uint256(1), ep);
        ...
        bytes memory pInitData =
            abi.encodeWithSelector(TokenPaymaster.initialize.selector, address(t), ep, oracle, ownerPM);
    ...
    function testFuzz_FlowStrict(int256 pr, uint256 mk, uint256 gasLimit) public {
        ...
        // gasLimit is bound to [100_000, 2_000_000] above, so it always fits in uint128.
        ...
        // pr is bound to [1e6, 1e15] (always positive), so the int256 -> uint256 cast cannot truncate.
        // forge-lint: disable-next-line(unsafe-typecast)
